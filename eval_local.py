...
# Per-request HTTP timeout. Default 600 (original local-harness value). The cloud
# grader streams GPQA (thinking) so it has no total-wall-clock cap; set a large
# value (e.g. 3600) to make this non-streaming local harness grader-faithful.
...
def _http_invoke(payload: dict, timeout: int = HTTP_TIMEOUT) -> dict:
...
def _invoke_chat(prompt, max_tokens, temperature=0.0, thinking=False):
    ...
    t0 = time.perf_counter()
    result = _http_invoke(payload, timeout=HTTP_TIMEOUT)
    latency_ms = (time.perf_counter() - t0) * 1000
    choices = result.get("choices", [])
        ...
        def _do(idx):
            ...
            for attempt in range(3):
                try:
                    ...
                    print(f"  [_do {idx}] attempt {attempt} ERR: {repr(e)[:300]}", flush=True)
...
def run_quality_eval():
    ...
    for task_name, result_key, num_fewshot, metric_key, threshold, thinking in QUALITY_TASKS:
        ...
        # <1.0 = fraction per task; >=1.0 = absolute #examples per task (balanced smoke); 0 = full
        limit = (QUALITY_LIMIT if 0 < QUALITY_LIMIT < 1.0
                 else int(QUALITY_LIMIT) if QUALITY_LIMIT >= 1.0 else None)
